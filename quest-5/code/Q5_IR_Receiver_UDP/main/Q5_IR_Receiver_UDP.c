/*
  ...
  Scooter: IR Receiver & UDP Client
  4/28/2023
*/
...
static void LED_task() {
    ...
    while (1) 
    {
        // If the button has been pressed (ON)
        if (strncmp(rx_buffer, FOBrandKEYreceived, 10) == 0) {
            ESP_LOGI(TAG, "LED Blinking, Scooter Unlocked!");
            sprintf(myColor, 'G');
        }
        // If the button is de-pressed (OFF)
        else {
...
//   xTaskCreate(udp_client_task, "udp_client", 4096, NULL, 5, NULL);
    ...
    xTaskCreate(LED_task, "LED", 4096, NULL, configMAX_PRIORITIES-1, NULL);
