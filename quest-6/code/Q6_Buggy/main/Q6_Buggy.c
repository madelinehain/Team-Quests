...
void vTask_actuateServo(){
    ...
    for(;;){
        // If close to front wall but not too close: TURN
        if (strncmp(rx_buffer, "Left", 4) == 0)  {
            servoSetpoint = 60;
            convertedPw = servoDegToPw(servoSetpoint); // convert the input angle given in degrees to the corresponding PWM...
            ESP_ERROR_CHECK(mcpwm_set_duty_in_us(MCPWM_UNIT_0, MCPWM_TIMER_0, MCPWM_OPR_A, convertedPw)); // set the pin to the correct PWM...
            printf("LEFT: Steering angle set to %d degrees\n", servoSetpoint); // alert the user that the change of angle has been successful...
            // printf("PWM Value: %d\n", convertedPw); // output the PWM value for debugging purposes...
            distanceRightFlag = false; // lower the servo flag. 
        }
        // If close to front wall but not too close: TURN
        else if (strncmp(rx_buffer, "Right", 5) == 0)  {
            servoSetpoint = -60;
            convertedPw = servoDegToPw(servoSetpoint); // convert the input angle given in degrees to the corresponding PWM...
            ESP_ERROR_CHECK(mcpwm_set_duty_in_us(MCPWM_UNIT_0, MCPWM_TIMER_0, MCPWM_OPR_A, convertedPw)); // set the pin to the correct PWM...
            printf("RIGHT: Steering angle set to %d degrees\n", servoSetpoint); // alert the user that the change of angle has been successful...
            // printf("PWM Value: %d\n", convertedPw); // output the PWM value for debugging purposes...
            distanceRightFlag = false; // lower the servo flag. 
        }
        ...
        else if(distanceRightFlag){ // If the user has sent an angle directed to the servo...
...
void vTask_actuateMotor(){
    ...
    float direction = 1.0;    // 1 --> forward, -1 --> backward

    float chosen_speed = 0.0;
    ...
    for(;;){
        ...
        ESP_LOGI(TAG, "---> Setting Speed to %f ,  motorSetpoint = %f", motorSetpoint, chosen_speed);
